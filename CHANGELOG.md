...
- **[수정] 히트맵 · 초대형주-대형주 격차로 인한 시인성 문제**: V6.0.24에서 타일 크기를 시가총액 기준으로 고쳤지만, 시가총액을 그대로(선형) 쓰다 보니 NVDA/AAPL(3조 달러대)과 QCOM/PLTR(1~2천억 달러대) 같이 같은 카테고리 안에서 격차가 20~30배 나는 경우 소형 대형주가 다시 점처럼 눌리는 문제가 있었습니다. 타일 크기를 시가총액의 **제곱근**으로 바꿔 격차를 압축했습니다(순서·상대적 크기 우위는 그대로 유지하되 극단적 비율만 완화). 적용 범위: `render_treemap(size_by="marketcap")`, `render_grouped_treemap()`, 나스닥100 시가총액 맵, 나스닥100 섹터별 맵 — 시가총액 기반 크기 배분을 쓰는 4곳 전부.
