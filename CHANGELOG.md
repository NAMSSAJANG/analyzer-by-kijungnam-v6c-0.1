...
- **포지션 사이징 계산기에 계산 방식 3가지 추가**: 기존에는 손절 거리 기반 고정 리스크 % 방식 하나뿐이었는데, 아래 3개 모드를 선택할 수 있도록 확장했습니다.
  - **정밀 모드 · 손절 기준**: 기존 계산 로직 그대로 유지 (손실 허용 비율 → 허용 손실 금액 → 주당 손절 리스크로 나눠 수량 계산).
  - **간편 모드 · 비중 기준**: 손절 거리·Risk 배율 개념 없이 "계좌의 몇 %를 쓸지"만 입력하면 수량이 나오는 가장 단순한 방식. 손절 시 손실 금액은 참고용으로 함께 보여줌. 계산기가 어렵다는 피드백을 반영.
  - **분할 매수 모드 · 3단계 진입**: 한 지점에 전량을 걸지 않고 3단계로 나눠 진입하는 계획을 계산. 손절가는 3단계 모두 동일하게 유지.
- **`SetupScore`에 `tranches` 필드 추가** (`core_models.py`): 각 셋업(Pullback/Momentum)마다 3단계 진입 가격·비중·설명을 담는 `EntryTranche` 3개를 갖도록 확장.
- **`setup_engine.py`에 분할 매수 가격 계산 로직 추가**:
  - 눌림목(Pullback): 지지 구간 상단→중심→하단 순으로 40/30/30 비중 배분 (동일한 손절가를 공유하는, 구간 폭 안에서의 분할 매수 — 무한정 물타기가 아니라 구간 자체의 무효화 손절선으로 하방이 제한됨).
  - 모멘텀(Momentum): 돌파 트리거 → +0.6 ATR → +1.4 ATR 순으로 40/30/30 비중 배분 (돌파가 유지·확장되는 것을 확인하며 추가하는 피라미딩 방식).
- 세 모드 모두 오프라인 시나리오(smoke_test 기반 momentum/pullback 픽스처)로 가격 산출값을 검증함.
